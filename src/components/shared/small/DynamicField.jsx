...
const MultiCheckboxInputType = ({ field, className, form, setForm }) => {
  ...
  const multiCheckBoxHandler = (e) => {
    if (form[name]?.includes(e.target.value)) {
      setForm({ ...form, [uniqueId]: { name: name, value: form[name].filter((item) => item !== e.target.value) } });
    } else {
      setForm({ ...form, [uniqueId]: { name: name, value: [...form[name], e.target.value] } });
    }
  };
  return (
    <div className={`flex w-full justify-between gap-8 ${className}`}>
...
const OtherInputType = ({ field, className, form, setForm, isConfirmField, suggestions = [] }) => {
  ...
  const onPlaceChanged = () => {
    ...
    setForm((prev) => ({ ...prev, [uniqueId]: { name: name, value: place.formatted_address } }));
  };
                ...
                <div className="relative">
                  <textarea
                    ...
                    onChange={(e) =>
                      setForm((prev) => ({
                        ...prev,
                        [uniqueId]: { name: name, value: e.target.value },
                      }))
                    }
