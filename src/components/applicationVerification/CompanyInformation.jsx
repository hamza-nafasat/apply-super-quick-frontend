...
function CompanyInformation({
  ...
  useEffect(() => {
    ...
    const isAllRequiredFieldsFilled =
      allFilled && isNaicsFilled && isCompanyStockSymbol && isSignatureDone;
  ...
  return (
    <div className="mt-14 h-full overflow-auto">
      {updateSectionFromatingModal && (
        <Modal
          isOpen={updateSectionFromatingModal}
          onClose={() => setUpdateSectionFromatingModal(false)}
        >
          <EditSectionDisplayTextFromatingModal
            step={step}
            setModal={setUpdateSectionFromatingModal}
          />
        </Modal>
      )}
      ...
      <div className="mb-10 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button
            onClick={() =>
              saveInProgress({ data: { ...form, naics: naicsToMccDetails }, name: sectionKey })
            }
          ...
          {isCreator && (
            <>
              ...
              />
              <Button
                onClick={() => setUpdateSectionFromatingModal(true)}
                label={'Update Display Text'}
              />
            </>
          )}
        </div>
      ...
      <div className="flex justify-end gap-4 p-4">
        <div className="mt-8 flex justify-end gap-5">
          {currentStep > 0 && (
            <Button variant="secondary" label={'Previous'} onClick={handlePrevious} />
          )}
          {currentStep < totalSteps - 1 ? (
            <Button
              ...
              label={
                isAllRequiredFieldsFilled || loadingNext
                  ? 'Next'
        ...
        <div className={`'mt-2' flex w-full gap-4`}>
          {naicsApiData?.otherMatches?.map((match, i) => (
            <button
              ...
              key={i}
              onClick={() => handlerOnClickOnOtherMatches(i)}
            >
              <input
                placeholder="NAICS Code and Description"
                type="text"
