...
const createUserTests = ({ firstName, lastName, email, role, password, uniqueEmail, failEmail }) => {
  ...
  it("should return 403 if email already exists", async () => {
    ...
    if (!user) user = await Auth.create({ firstName, lastName, email, password, role });
  ...
  it("should return 400 if user creation fails (simulate DB failure)", async () => {
    ...
    const res = await request(app)
      ...
      .send({ firstName, lastName, email: failEmail, password, role });
    expect(res.statusCode).toBe(400);
  ...
  it("should create user successfully ", async () => {
    const existing = await Auth.findOne({ email: uniqueEmail });
    if (existing) await Auth.findByIdAndDelete(existing?._id);
    const res = await request(app)
      .post(createUserRoute)
      .send({ firstName, lastName, email: uniqueEmail, password, role });
    expect(res.statusCode).toBe(201);
    expect(res.body.message).toBe("User Created Successfully");
    const userInDb = await Auth.findOne({ email: uniqueEmail });
